...
public class MarkLowAndHighFreKmer {
	...
	public static void main(String[] args) throws IOException {
        ...
        /*int KmerErrorArraySize=getLine(KmerFilterPath+"FilterErrorFre.short1.txt");
        String [] KmerErrorArray=new String[KmerErrorArraySize];
        int CountKmerErrorArray=LoadingData(KmerFilterPath+"FilterErrorFre.short1.txt",KmerErrorArray);		
        ...
        int KmerLowArraySize=getLine(KmerFilterPath+"FilterLowFre.short1.txt");
        String [] KmerLowArray=new String[KmerLowArraySize];
        int CountKmerLowArray=LoadingData(KmerFilterPath+"FilterLowFre.short1.txt",KmerLowArray);		
        ...
        int KmerRepeatArraySize=getLine(KmerFilterPath+"FilterRepeat.short1.txt");
        String [] KmerRepeatArray=new String[KmerRepeatArraySize];
        int CountKmerRepeatArray=LoadingData(KmerFilterPath+"FilterRepeat.short1.txt",KmerRepeatArray);		
		String [] LineSplitMin = KmerRepeatArray[0].split("\t\t|\\s+|\n");
        int HighFreMin=Integer.parseInt(LineSplitMin[0]);
	    //Store SDK File
		int Count=0;
		int dskSize=getLine(KmerFrePath+"short1.txt");
	    File file = new File(KmerFrePath+"short1.txt");
	    if (file.isFile() && file.exists()) 
		{
			 InputStreamReader read = new InputStreamReader(new FileInputStream(file), encoding); // set format of encoding.
			 BufferedReader bufferedReader = new BufferedReader(read);
			 while ((readtemp = bufferedReader.readLine()) != null) 
			 {
				   ...
				   {
					   	FileWriter writer1= new FileWriter(KmerFilterPath+"ErrorFreKmer.short1.txt",true);
                        writer1.write(readtemp+"\n");
                        writer1.close();
				   }*/
				   if(LowFreCompare<=LowFreMax)  
				   {
					   	FileWriter writer1= new FileWriter(KmerFilterPath+"LowFreKmer.short1.txt",true);
                        writer1.write(readtemp+"\n");
                        writer1.close();
				   }
                   if(LowFreCompare>=HighFreMin)
				   {
					   	FileWriter writer2= new FileWriter(KmerFilterPath+"RepeatKmer.short1.txt",true);
